...
def make_driver() -> webdriver.Chrome:
    ...
    opts.add_argument("--window-size=2400,2400")
...
def render_block(driver: webdriver.Chrome, code: str, out_path: Path) -> None:
    ...
    size = elem.size
    width = max(int(size["width"]) + 80, 800)
    height = max(int(size["height"]) + 80, 400)
